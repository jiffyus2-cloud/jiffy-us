      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            {t('mug.uploadDesc') || 'Sube las fotos para tus tazas. Cada foto creará un diseño independiente.'}
          </p>
        </div>
    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8 sticky top-24 bg-white/95 backdrop-blur-sm z-40 py-4 border-b">
        <div>
          ...
          <p className="text-gray-500">{safeItems.length} tazas en tu colección</p>
        </div>
        <button
          onClick={onComplete}
          className="px-8 py-3 bg-black text-white rounded-full hover:bg-gray-800 transition-all shadow-lg font-medium"
        >
          {t('organizer.complete') || 'Continuar al Checkout'}
        </button>
      </div>
                         ...
                         <div className="absolute inset-0 flex flex-col items-center justify-center">
                            ...
                            {item.photos[0] ? (
                               ...
                               <div className="text-gray-300 flex flex-col items-center gap-2">
                                  ...
                                  <span className="text-xs font-bold uppercase tracking-widest text-gray-400">Área de Diseño</span>
                               </div>
                            )}
                            
                            {/* LOGICA DE TEXTO Y RECORTES */}
                            {item.designStyle === 'text-cutout' ? (
                               // Efecto "Recorte de texto"
                               <div className="absolute inset-0 bg-white pointer-events-none mix-blend-screen flex items-center justify-center p-6 z-10">
                                  ...
                                  >
                                     {item.text || (editingItemId === item.id ? 'ESCRIBE AQUÍ' : '')}
                                  </div>
                               </div>
                            ) : (
                               item.text && (
                                  <div 
                                    className="absolute inset-0 flex items-center justify-center p-10 pointer-events-none z-10"
                                    style={{
                                       fontSize: `${item.fontSize}px`,
                                       fontFamily: item.fontFamily,
                                       fontWeight: item.fontWeight || 400, // <-- Aplica grosor
                   ...
                   <div>
                      ...
                      <div className="space-y-6">
                         ...
                         <div>
                           ...
                           <div className="flex gap-2">
                             <button
                               ...
                               onClick={() => updateItemText(item.id, { designStyle: 'separate' })}
                               className={`flex-1 py-3 px-2 rounded-xl border-2 text-[10px] font-bold uppercase transition-all ${
                                 item.designStyle !== 'text-cutout'
                             ...
                             <button
                               disabled={editingItemId !== item.id}
                               onClick={() => updateItemText(item.id, { designStyle: 'text-cutout' })}
                               className={`flex-1 py-3 px-2 rounded-xl border-2 text-[10px] font-bold uppercase transition-all ${
                                 item.designStyle === 'text-cutout'
                                   ? 'border-black bg-black text-white shadow-md'
                                   : 'border-gray-200 bg-white text-gray-500 hover:border-black'
                         ...
                         >
                            ...
                            <span className="font-bold uppercase text-xs">{item.photos[0] ? 'Cambiar Foto' : 'Añadir Foto'}</span>
                         </button>
                         ...
                         <div className="relative">
                            ...
                            >
                               <div className="flex items-center gap-3">
                                  ...
                                  <span className="font-bold uppercase text-xs truncate max-w-[200px]">
                                     {item.text || 'Añadir Texto'}
                                  </span>
                               </div>
      ...
      <div className="mt-20 text-center pb-20">
        ...
        >
          ...
          <span className="text-lg font-medium">Añadir otra Taza</span>
        </button>
      </div>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <Type className="w-5 h-5" />
                <h4 className="text-xl font-bold">Editar Texto</h4>
              </div>
              <button
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase mb-2 block items-center gap-1.5 truncate">
                    <ALargeSmall className="w-3.5 h-3.5 inline" /> Tamaño
                  </label>
                  <select
                    value={currentEditingItem.fontSize}
                    onChange={(e) => updateItemText(currentEditingItem.id, { fontSize: parseInt(e.target.value) })}
                    className="w-full h-[42px] px-2 border-2 border-gray-100 rounded-xl focus:outline-none focus:border-black bg-white font-bold text-sm"
                  >
                ...
                <div className="col-span-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase mb-2 block truncate">Fuente</label>
                  <select
                    value={currentEditingItem.fontFamily}
                    onChange={(e) => updateItemText(currentEditingItem.id, { fontFamily: e.target.value })}
                    className="w-full h-[42px] px-1 border-2 border-gray-100 rounded-xl focus:outline-none focus:border-black bg-white font-bold text-sm"
                  >
                ...
                <div className="col-span-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase mb-2 block truncate">Grosor</label>
                  <select
                    value={currentEditingItem.fontWeight || 400}
                    onChange={(e) => updateItemText(currentEditingItem.id, { fontWeight: parseInt(e.target.value) })}
                    className="w-full h-[42px] px-1 border-2 border-gray-100 rounded-xl focus:outline-none focus:border-black bg-white font-bold text-sm"
                  >
