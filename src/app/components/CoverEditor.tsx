...
import { Upload, X, Check, Layout, Type, Image as ImageIcon, Palette } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-white z-[100] flex flex-col md:flex-row h-screen animate-in fade-in duration-300 overflow-hidden">
      <div className="h-[35vh] md:h-full md:flex-1 bg-[#F3F4F6] flex items-center justify-center p-2 md:p-16 relative order-1 md:order-2 border-b md:border-b-0 border-gray-200 shrink-0">
        <button 
          onClick={onClose} 
          className="hidden md:flex absolute top-10 right-10 p-3 bg-white rounded-full shadow-2xl hover:scale-110 transition-all z-20 group"
        >
        ...
        <div className="hidden md:flex absolute right-10 bottom-10 bg-black/10 backdrop-blur-md px-5 py-2.5 rounded-full items-center gap-4">
          <div className="flex flex-col">
            ...
            <span className="text-xs font-bold text-black">{coverSize} CM</span>
          </div>
          <div className="w-px h-5 bg-black/10" />
          <div className="flex flex-col">
            <span className="text-[8px] font-black uppercase text-black/50 leading-none">Ratio</span>
            <span className="text-xs font-bold text-black">{aspectRatio}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 w-full md:w-[400px] md:h-full border-r border-gray-200 flex flex-col bg-white shadow-xl z-10 order-2 md:order-1 overflow-hidden">
        <div className="p-3 md:p-6 border-b border-gray-100 flex items-center justify-between shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors md:hidden">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-3 md:grid-cols-2 gap-2">
              {Array.from({ length: numLayouts }, (_, i) => i + 1).map((layout) => (
                <button
                  ...
                  className={`group relative py-2 md:py-3 px-1 rounded-lg md:rounded-xl text-xs font-bold transition-all border-2 ${
          ...
          <section>
            <div className="flex items-center gap-1.5 mb-3 text-gray-400">
              <Palette size={16} />
              <h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest">Color de Tipografía</h3>
            </div>
          ...
          <section className="space-y-4 md:space-y-5">
            <div className="flex items-center gap-1.5 mb-2 text-gray-400">
              <Type size={16} />
              <h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest">Contenido del Texto</h3>
            </div>
            ...
            <div className="space-y-3 md:space-y-4">
              <div className="space-y-1 md:space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border-2 border-gray-50 p-2.5 md:p-3.5 rounded-lg focus:bg-white focus:border-black outline-none transition-all font-bold text-sm md:text-base"
              ...
              {!( (isSquare || isHorizontal) && selectedLayout === 5 ) && (
                <div className="space-y-1 md:space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-50 border-2 border-gray-50 p-2.5 md:p-3.5 rounded-lg focus:bg-white focus:border-black outline-none transition-all font-medium text-xs md:text-sm"
        ...
        <div className="p-3 md:p-6 border-t border-gray-100 bg-gray-50/50 flex gap-2 shrink-0">
          <button 
            onClick={onClose}
            className="flex-1 py-2.5 md:py-3.5 bg-white text-black border-2 border-black font-black uppercase tracking-tighter md:tracking-widest rounded-lg md:rounded-xl hover:bg-gray-50 transition-all text-[10px] sm:text-xs"
          >
            Cerrar
          </button>
          <button 
            onClick={() => onSave({ coverImage, coverTitle, coverSubtitle, coverYear, selectedLayout, coverCrop, typographyColor })}
            className="flex-[2] py-2.5 md:py-3.5 bg-black text-white font-black uppercase tracking-tighter md:tracking-widest rounded-lg md:rounded-xl flex items-center justify-center gap-1.5 md:gap-2 hover:bg-zinc-800 transition-all shadow-lg active:scale-[0.98] text-[10px] sm:text-xs"
          >
