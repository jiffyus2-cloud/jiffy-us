...
const CoverEditor: React.FC<CoverEditorProps> = ({ 
  ...
  const numericAspectRatio = isVertical ? 21/28 : isHorizontal ? 28/21 : 1;
    ...
    <div className="fixed inset-0 bg-white z-[100] flex flex-col md:flex-row h-screen animate-in fade-in duration-300 overflow-hidden">
      <div className="h-[35vh] md:h-full md:flex-1 bg-[#F3F4F6] flex items-center justify-center p-2 md:p-16 relative order-1 md:order-2 border-b md:border-b-0 border-gray-200 shrink-0">
        ...
        <div className="hidden md:flex absolute right-10 bottom-10 bg-black/10 backdrop-blur-md px-5 py-2.5 rounded-full items-center gap-4">
          ...
          <div className="flex flex-col"><span className="text-[8px] font-black uppercase text-black/50 leading-none">Ratio</span><span className="text-xs font-bold text-black">{aspectRatio}</span></div>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8">
          ...
          {!hidePhoto && (
            <section className="pb-4">
               ...
               {coverImage ? (
                  <div className="relative group rounded-xl md:rounded-2xl overflow-hidden shadow-md w-full max-w-[200px] md:max-w-none mx-auto border border-gray-100" style={{ aspectRatio: aspectRatio }}>
                    <ImageCropper 
                      src={coverImage} 
                      position={coverCrop} 
                    />
      ...
      {isCropModalOpen && (
        <CropModal isOpen={true} onClose={() => setIsCropModalOpen(false)} imageSrc={coverImage} currentCrop={coverCrop} aspectRatio={numericAspectRatio} title="Ajustar Portada del Álbum" onSave={setCoverCrop} />
      )}
    </div>
  );
};
